...
class V2Vchannels:              
    ...
    def calculate_reliability(self, dij, t, delta_t, sigma_vij, mu_vij):
        if delta_t <= 0 or t <= 0:
            return 0.0
        
        term1 = (((2 * dij) / t) - mu_vij) / (np.sqrt(2) * sigma_vij)
        term2 = (((2 * dij) / (t + delta_t)) - mu_vij) / (np.sqrt(2) * sigma_vij)
        rlij = erf(term1 - term2)
        return max(0.0, rlij)
...
class Environ:
    ...
    def __init__ (self,down_lanes, up_lanes, left_lane, right_lane, width, height, n_Veh=80):
        self.timestep = 0.01
    ...
    def renew_positions(self):
        ...
        while(i < len(self.vehicles)):
            ...
            i += 1
    def test_channel(self,n_Veh=80):
        # ===================================
